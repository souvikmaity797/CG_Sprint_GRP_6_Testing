...
public class LabTest {
    ...
    public void each_search_result_should_contain_expected_keywords_from(String testCaseId) {
        ...
        for (int i=0;i<5;i++) {
            String lower = results.get(i).toLowerCase();
            Assert.assertTrue(lower.contains(k1) || lower.contains(k2)
                            || lower.contains(k3) || lower.contains(k4),
                    "Result '" + results.get(i) + "' did not match keywords ["
                            + k1 + ", " + k2 + ", " + k3 + ", " + k4 + "]");
        }
    }
    ...
    public void add_button_should_be_functional_for_each_result() {
        Assert.assertFalse(labTestPage.searchResultText().isEmpty(),
                "No results found to check Add buttons.");
    }
    ...
    public void payment_should_fail() {
        Assert.assertNotNull(circleMemPaymentPage.errorMsg(),
                "Payment did not fail – no error message found.");
    }
    ...
    public void membership_should_not_be_activated() {
        Assert.assertFalse(driver().getCurrentUrl().contains("membership/success"),
